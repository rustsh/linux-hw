...
Так как необходимо снимать резервные копии каталога **/etc**, запуск Borg производится под пользователем root.
...
1. На обеих машинах устанавливается Borg — из официального репозитория скачивается бинарный файл и сохраняется в каталог **/bin**, ему выдаются права на выполнение (0755).
    ...
    > *Примечание*. Если скопировать файл в каталог **/usr/local/bin**, как это указано на официальном сайте, то запуск программы через Ansible вызовет ошибку, так как этого каталога нет в переменной `$PATH` пользователя root, под которым выполняется плейбук (не помогает даже использование ключа `become_user`).
...
4. На бэкап-сервере создаётся директория для хранения бэкапов: **/opt/backups/files-etc**.
...
6. Параметризированный скрипт для снятия бэкапа [borg-etc.sh](provisioning/roles/run_borg/templates/borg-etc.sh.j2) копируется в каталог **/opt** на клиенте, ему выдаются права на выполнение.
    ...
    В скрипте задаётся нужная политика хранения бэкапов при помощи ключей команды `borg prune`: `--keep-within 30d` и `--keep-monthly 2`. Данные параметры применяются последовательно, то есть Borg будет хранить по одному архиву за предыдущие два месяца, не считая бэкапы, созданные в течение последних 30 дней.
