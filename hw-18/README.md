...
2. На хосте inetRouter настраивается iptables:
   ...
   - удаляются все правила для таблицы filter:

        ```console
        iptables -F
        ```

   - конфигурация iptables сохраняется для работы после перезагрузки:
...
4. На роутерах включается форвардинг пакетов: в каталог **/etc/sysctl.d** добавляется файл **forwarding.conf**, содержащий следующую строчку:
