...
- [Как проверить выполнение скрипта](#Как-проверить-выполнение-скрипта)
...
11. Создаётся новая переменная, в которую при помощи программы awk запоминаются коды возврата. Если в подстроке запроса не три поля, а одно, то код возврата содержится не в девятом поле всей строки файла, а в седьмом. Такой случай также обрабатывается awk:
...
### Как проверить выполнение скрипта
...
1. При помощи команды sed проставить в файле [access-4560-644067.log](access-4560-644067.log) дату, когда запускается скрипт, например:
    ...
    ```
    Необходимо учитывать, что в конце файла содержатся данные за следующий день, и обработать их тоже, иначе они исказят выборку:
...
4. При помощи команды `crontab -e` добавить в файл конфигурации cron выполнение скрипта каждый час. Так как в скрипте прописаны относительные пути, также следует предусмотреть переход в каталог, содержащий скрипт, например:
...
5. Спустя какое-то время (достаточное для нескольких запусков по расписанию) проверить входящую почту при помощи команды `sudo mail` (если получателем был выбран текущий пользователь, то команда `sudo` не нужна):
    ...
    <details>
    <summary>Посмотреть содержимое письма</summary>
...
6. Также можно проверить содержимое файлов **lastrun.txt** и **result.txt**:
    ...
    <details>
    <summary>Посмотреть результат выполнения команды <code>cat result.log</code></summary>
...
7. После проверки выполнение по расписанию можно отключить, открыв файл конфигурации командой `crontab -e` и удалив соответствующую строку. Если других заданий нет, то вместо этого достаточно выполнить команду `crontab -r`.
...
5. Вернуться в первое окно с терминалом (или, если была нажата комбинация **CTRL + Z**, запустить процесс командой `fg`) и прервать выполнение скрипта комбинацией **CTRL + C**.
